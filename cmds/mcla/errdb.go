...
func (db *ghErrDB)fetch(subpaths ...string)(res *http.Response, err error){
	var path string
	if path, err = url.JoinPath(db.Prefix, subpaths...); err != nil {
		return
	}
	if res, err = http.DefaultClient.Get(path); err != nil {
		return
	}
	return
}
...
func (db *ghErrDB)fetchGhDBVersion()(v versionDataT, err error){
	...
	if res.StatusCode != 200 {
		err = &HTTPStatusErr{ res.Request.URL.String(), res.StatusCode }
		return
	}
...
func (db *ghErrDB)getErrorDesc(id int)(desc *mcla.ErrorDesc, err error){
...
func (db *ghErrDB)ForEachErrors(callback func(*mcla.ErrorDesc)(error))(err error){
